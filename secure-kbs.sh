...
kubectl patch deploy -n coco-tenant kbs --type='json' -p='[{"op": "add", "path": "/spec/template/spec/volumes/0", "value": {"name": "https", "secret": {"secretName" : "kbs-https"}}}, {"op": "add", "path": "/spec/template/spec/containers/0/volumeMounts/0", "value": {"name": "https", "mountPath": "/etc/kbs/https"}}]'

svc_port="$(kubectl get svc kbs-nodeport -n coco-tenant -o jsonpath="{.spec.ports[*].nodePort}")"
